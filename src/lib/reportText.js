import { reconcileMetrics, round2, toNumber, topProducts } from "./calc";
...
function shortDate(date) {
  return date.length >= 10 ? `${Number(date.slice(5, 7))}月${Number(date.slice(8, 10))}日` : date;
}

export function buildWeChatText(report, metrics, settings, version = "shareholder") {
  const parts = [];
  const isDay = (report.periodType || "day") === "day";
  const dateLabel = isDay ? shortDate(report.date) : `${shortDate(report.date)}~${shortDate(report.endDate || report.date)}`;
  const store = report.storeName || settings.storeName || "UU台球俱乐部";

  parts.push(`【${store} · ${dateLabel}${isDay ? "营收日报" : "经营报告"}】`);
  parts.push("");
  parts.push(`本期总营收：¥${money(metrics.total)}`);
  ...
  } else if (metrics.dailyAverage !== null) {
    parts.push(`日均营收 ¥${money(metrics.dailyAverage)}`);
  }
  parts.push("");
  ...
  if (version !== "staff") {
    ...
    parts.push("【对账结论】");
    parts.push(`总营收（经营收入）¥${money(metrics.total)}（现场营业额+团购核销净额）｜ 储值充值 ¥${money(report.member.rechargeAmount)}（预收款）`);
    parts.push(
    ...
    parts.push(`累计差额（全部日报 ${metrics.reconcileCount} 份）¥${money(metrics.reconcileTotal)}`);
    ...
    if (report.reconciliation?.systemError) parts.push(`银行流水备注：${report.reconciliation.systemError}`);
  }
  parts.push("");

  parts.push(`【${isDay ? "昨日完成" : "本期完成"}】`);
  const done = lines(report.done);
  parts.push(done.length ? numbered(done) : "无");
  parts.push("");

  parts.push(`【${isDay ? "今日待办" : "下期待办"}】`);
